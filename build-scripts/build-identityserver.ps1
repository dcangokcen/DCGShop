# Build: dcgshop-identityserver → ghcr.io/dcgshoptest/dcgshop-identityserver (linux/arm64)
param([string]$Registry = "ghcr.io/dcgshoptest", [switch]$Push)
$Tag = if ($Registry) { "$Registry/dcgshop-identityserver" } else { "dcgshop-identityserver" }
$buildArgs = @("buildx","build","--platform","linux/arm64","-f","IdentityServer/DCGShop.IdentityServer/Dockerfile","-t",$Tag)
