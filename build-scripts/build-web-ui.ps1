# Build: dcgshop-web-ui → ghcr.io/dcgshoptest/dcgshop-web-ui (linux/arm64)
param([string]$Registry = "ghcr.io/dcgshoptest", [switch]$Push)
$Tag = if ($Registry) { "$Registry/dcgshop-web-ui" } else { "dcgshop-web-ui" }
$buildArgs = @("buildx","build","--platform","linux/arm64","-f","Frontends/DCGShop.WebUI/Dockerfile","-t",$Tag)
